...
export class Input {
  constructor() {
    ...
    document.addEventListener("keydown", (e) => {
      ...
      if (e.code === "Digit1") {
        this.onDigitPressed(ONE);  
      }	
      if (e.code === "Digit2") {
        this.onDigitPressed(TWO);  
      }
    ...
    document.addEventListener("keyup", (e) => {
      ...
      if (e.code === "Digit1") {
        this.onDigitReleased(ONE);  
      }
      if (e.code === "Digit2") {
        this.onDigitReleased(TWO);  
      }      
    })
  ...
  get digit() {
    return this.heldDigits[0];  
  }  
  onDigitPressed(number) {
    if (this.heldDigits.indexOf(number) === -1) {
      this.heldDigits.unshift(number); 	
    }
  } 
  onDigitReleased(number) {
    const index = this.heldDigits.indexOf(number);
    if (index === -1) {
      return;	
    }
    this.heldDigits.splice(index, 1)
  }
  get direction() {
