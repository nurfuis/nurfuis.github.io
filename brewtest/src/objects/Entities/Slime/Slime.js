...
export class Slime extends GameObject {
  constructor(x, y, tileData) {
    ...
    events.on("COLLISION_DETECTED", this, playerData => {    
      this.onEnergyShield();
    });    
  }
  ready() {
  ...
  tryMove(delta) {
    ...
    if (currentStepData.direction) {  
      ...
      if (isSpaceFree(colliders.colliders, nextX, nextY, this.entityId).collisionDetected === false) {
        ...
        // Collision tile
        this.isMoving = true;
        this.lastSquare = new Vector2(Math.round(this.position.x), Math.round(this.position.y));
        
        this.nextSquare = new Vector2(Math.round(nextX), Math.round(nextY));   
