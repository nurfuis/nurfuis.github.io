class Camera extends GameObject {
    constructor(canvas, world) {
        ...
        events.on("CAMERA_SHAKE", this, (event) => {
            this.hasShake = true;
            const shakeX = event.position.x - event.targetPosition.x;
            const shakeY = event.position.y - event.targetPosition.y;
    ...
    panCamera(keysPressed) {
        ...
        if (keysPressed.includes('Numpad4')) { // Numpad 4 (Left)
            newX += this.panSpeed;
        }
        if (keysPressed.includes('Numpad6')) { // Numpad 6 (Right)
            newX -= this.panSpeed;
        }
        if (keysPressed.includes('Numpad8')) { // Numpad 8 (Up)
            newY += this.panSpeed;
        }
        if (keysPressed.includes('Numpad2')) { // Numpad 2 (Down)
            newY -= this.panSpeed;
        }
        if (keysPressed.includes('Numpad7')) { // Numpad 7 (Up Left)
            newX += this.panSpeed;
            newY += this.panSpeed;
        }
        if (keysPressed.includes('Numpad9')) { // Numpad 9 (Up Right)
            newX -= this.panSpeed;
            newY += this.panSpeed;
        }
        if (keysPressed.includes('Numpad1')) { // Numpad 1 (Down Left)
            newX += this.panSpeed;
            newY -= this.panSpeed;
        }
        if (keysPressed.includes('Numpad3')) { // Numpad 3 (Down Right)
            newX -= this.panSpeed;
            newY -= this.panSpeed;
        }
